...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel private (offset, local_i, rgb) reduction(+:pixel_array[:area])
    {
      int offset = (n/omp_get_num_threads()) * omp_get_thread_num();
      local_i = 0;
      rgb = calloc(3, sizeof(unsigned char));
      for (int i = 0; i < n/omp_get_num_threads(); i++)
          for (int j = 0; j < largura*3; j+=3){
              compute_julia_pixel(j/3, (i+offset), largura, altura, 1.0, rgb);
              printf("Thread: %d, RGB: %d %d %d, PIXEL: %d %d\n", omp_get_thread_num(),rgb[0],rgb[1],rgb[2], (i+offset), j/3);
              #pragma omp critical
              {
                pixel_array[local_i + offset]= rgb[0]; local_i++;
                pixel_array[local_i + offset]= rgb[1]; local_i++;
                pixel_array[local_i + offset]= rgb[2]; local_i++;
              }
