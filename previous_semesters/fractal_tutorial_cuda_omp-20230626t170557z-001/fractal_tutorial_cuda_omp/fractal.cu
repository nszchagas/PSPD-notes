...
__device__ void compute_julia_pixel(int x, int y, int largura, int altura, float tint_bias, unsigned char *rgb) {
...
__global__ void compute(int largura, int altura_total, unsigned char *pixel_array) {
  int i = blockIdx.x;
  unsigned char rgb[3];
...
int main(int argc, char *argv[]) {
  ...
  FILE *output_file;
  unsigned char *pixel_array, *pixel_array_dev;

  if ((argc <= 1) | (atoi(argv[1]) < 1)) {
    fprintf(stderr, "Entre 'N' como um inteiro positivo! \n");
    return -1;
  }
  n = atoi(argv[1]); altura = n; largura = 2 * n; area = altura * largura * 3;
  ...
  compute<<<n, 1>>>(largura, altura, pixel_array_dev);
